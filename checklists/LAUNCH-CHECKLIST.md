...
- [~] **Phase 2.5 — Empty-field trade derivation — ROLLED BACK 2026-05-09** (`95ccfb1` build → `197805e` + `cc0f845` rollback + migration `migrate-strip-primary-trade-id`). The single-trade-per-sub model collapsed across taxonomy switching (Tony Tile maps to "Tile" in GC but no name match in CSI MasterFormat; exact-name-match bridge silently soft-fails cross-taxonomy) AND the line item creation flow has the trade dropdown visually before the sub picker (users pick trade manually before derivation fires). Half-done autofill is worse than no autofill — Bari learns to distrust the visual cue. Rolled back per **DEC-218 (Half-done isn't done)**. Auto-derivation deferred to Phase 3+ as proper per-taxonomy mapping work. Two scaffolding pieces preserved (`deriveLineTrade` pure function + `peopleMap` extension on `useWorkspacePeople`).
- [x] **Drill-through Seedlings A+B — closed 2026-05-09 afternoon** (`472583c` hotfix + `14ba199` build). Six Desk Home tiles drill through to source records via `<ProjectTileDrillIn>` primitive (no extraction owed, primitive already shipped). Hotfix added direction filter to `paymentsReceived` aggregation (was including paid-out FSPayment with `direction: 'paid'` AND `status: 'received'` lifecycle marker into "Received income"). `useConsumePrefill(key)` hook extracted (Living Feet — 5+ inline consumers funneled through one hook); migrated trivial cases (Estimates + Log) where 3-line swap was clean; Documents inline left in place (multi-key conditional doesn't compose). "This Month" tile renamed "Spent This Month" (its actual semantic is outgoing materials/labor cost, not income). Three new prefill keys wired (`fs-people-prefill-client-id`, `fs-people-prefill-worker-id`, `fs-projects-prefill-project-id` + `fs-projects-prefill-payment-id` paired). ~9 min wallclock for full build (16-30x faster than 3-5h estimate).
- [x] **Phase 1.0 commit 1 — line-item attribution + per-line rollup + projectSpent honest math + soft-delete schema** (2026-05-09 afternoon, `04162a8`). 12 files +746/-42; ~13 min wallclock vs 3.5-5h estimate (~16-23x faster). New: `src/utils/softDelete.js` (`excludeDeleted` helper), `src/hooks/useContractLineItems.js` (chronological union of estimate + signed CO line items with dev warning on missing ids per spec §10), `src/components/fieldservice/LineItemPicker.jsx` (type-ahead picker composing shadcn Command + Popover; graceful degradation for no-project / no-estimate / orphaned-line cases), `base44-prompts/PHASE-1-0-LINE-ITEM-ATTRIBUTION-AND-SOFT-DELETE.md`. Modified: FieldServiceLog.jsx (LineItemPicker integration on Sub Payment + Client Payment forms; `line_item_id` on FSPayment payload), FieldServiceProjects.jsx (largest — workspace allPayments query, projectSpent + spendByProject semantic update including FSPayment(paid), per-line rollup view section, Spent tile caption, drill-in updates, soft-delete sweep), six other files for soft-delete sweep at 27 sites total. Schema applied via `PHASE-1-0-LINE-ITEM-ATTRIBUTION-AND-SOFT-DELETE.md` Base44 prompt (Gate 1 clean, auto-commit `22ed68f` — 10 fields added across 4 entities). Doron verified working in production: LineItemPicker visible on Sub Payment + Client Payment forms; per-line rollup renders Estimated/Billed/Cost/Variance per line + Unallocated row at bottom; Spent tile gains "Materials + labor + sub payments" caption.
- [~] **Phase 1.0 commit 1.5 — Material/Labor attribution + per-line rollup expansion** — spec ratified at `0dbe54e` (Spec-Repo) covering FSMaterialEntry.line_item_id + FSLaborEntry.line_item_id + per-row LineItemPicker on Daily Log materials + labor entry rows + per-line rollup view expansion. Build prompt + Base44 prompt drafted (`base44-prompts/PHASE-1-0-COMMIT-1-5-MATERIAL-LABOR-ATTRIBUTION.md`). **DEFERRED to next session** — Base44 schema agent failed at 16:11 PT with timeout/error (Request ID `0432acde-c8e7-42d4-b92d-8d5947f6fb3c`); both writes rolled back per Doron's entity browser verification. KI #28. Retry when Base44 platform recovers.
- [ ] **Phase 1.0 commit 2 — edit/delete on cost-tracking entities + cleared-payment immutability + Reverse minimum-viable + cascade-delete server function** — spec ratified (LOG-LINE-ITEM-ATTRIBUTION-PROPOSAL §13 + §14 in Option Z phasing per patch 002). **BLOCKED on KI #28.** Gate 2 (rls.update removal on all four cost-tracking entities — FSPayment, FSMaterialEntry, FSLaborEntry, FSDailyLog) gates `withAuditLog` + `softDeleteWithCascade` server function asServiceRole writes per DEC-215. Build cannot ship until Base44 Security panel recovers AND Gate 2 applies cleanly. Realistic ~3.5-5h Hyphae build per audit (form-mode refactor for FSPayment edit + cascade-server-function atomicity testing).
- [ ] **Phase 2.6.1 — Line item entry redesign (Estimates surface only)** — single entry-point + collapsible trade groups + @dnd-kit drag-and-drop reorder + inline edit + signed-doc confirm-dialog lockdown UX + soft-delete-with-undo. Three-component decomposition (`<LineItemForm>` + `<LineItemsByTrade>` + `<LineItemRow>`). Spec at `Spec-Repo/spaces/field-service/PHASE-2-6-LINE-ITEM-ENTRY-REDESIGN.md` (`2b8686d`). Build queued. Bundles `parseWorkers` deduplication via `peopleMap` as polish target (4 inline `parseWorkers` helpers across `FieldServiceProjects.jsx` + `FieldServiceLog.jsx` migrate to `useWorkspacePeople(profile).peopleMap`).
- [ ] **Phase 2.6.2 — Line item entry redesign CO parity** — FSChangeOrder schema additions (`group_by_trade`, `taxonomy_preset_id`, `trade_categories_snapshot`, `trade_group_order`) + backfill migration + CO surface turn-on with same UX as Estimates. MIGRATION_SECRET kept live from Phase 2.5 rollback (single-secret protocol across sequential migrations). Builds on 2.6.1.
