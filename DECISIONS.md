...
> **Mirror sync (2026-06-10, per DEC-182).** Canonical: `Spec-Repo/platform/DECISIONS-200-249.md`. This pass clears the owed drift: DEC-236 through DEC-241 mirrored below in full (DEC-237 includes its 2026-06-09 amendment; DEC-238 carries its 2026-06-10 status update — merged + applied + sealed by DEC-241). Spec-Repo remains the source; this file mirrors unidirectionally.

### DEC-236: Events Shell — the Owner-Side Event Room on the Business Container; Canonical-Column Collapse + One RSVP Table (2026-06-08)

**Date:** 2026-06-08
**Status:** Active. Ratified at the Events-shell build on branch `feat/events-shell` (locallane `fe44324`, pushed to origin — **NOT merged to `main`**; staged behind the Construction Gate). Migration `20260608120000_create_events.sql` applied to remote via `npx supabase db push` (dry-run → apply, clean). Planted → Living on Doron's walk (create an event → RSVP → see the count). Paired planner: `MYCELIA-PLAN-EVENTS-SHELL`. Paired builder: `HYPHAE-BUILD-EVENTS-SHELL-COMPLETE`.

**Context:** Events is one of the four city surfaces named in REBUILD-SCOPE §2.2 (Directory / Events / Personal / Business). This build plants the OWNER-SIDE half — a business managing its own events — as the first concrete Events surface, deferring the public discovery page (the city-level read) to a fast-follow. The authoritative spec is the captured community-node Event entity + EventsWidget/EventEditor (the reference library, DEC-223 — lessons port, code does not). That capture is alias-heavy (the Base44 era accreted multiple representations of one concept) and carries an EventRSVP-vs-RSVP split bug. VERIFY-FIRST surfaced a framing tension: the `lib/tiles.ts` "Events" tile is the CITY-level "what's happening this week" home tile (no business in context) — the deferred PUBLIC calendar — not the owner room; the room tiles that sit "alongside Desk/Profile/Finance/Settings" live inline on the business hub.

**Decision:**
- **(a) Events is a BUSINESS-CONTAINER ROOM, a peer to Desk.** It mounts at `/businesses/[id]/events` as a 5th tile on the business hub (`app/(authenticated)/businesses/[id]/page.tsx`), sibling to Profile/Settings/Desk/Finance — NOT a Desk-internal room (not under `/desk/`), and NOT the city-level public calendar. The city-level home Events tile stays `inDevelopment` (lighting it would route to the deferred public page — dishonest navigation per DEC-209). This is a Framing-Question Pushback (MWP Step 0.5) resolution — the faithful reading of "a real room alongside Desk/Profile/Finance/Settings."
- **(b) Collapse every Base44 alias to ONE canonical column.** start/end (`date`+`end_date`+`duration_minutes`+`is_all_day`) → `starts_at` + `ends_at` (timestamptz instants; the editor's duration-vs-end-time toggle computes ends_at); location (`location`+`is_location_tbd`+`lat`/`lng`+`is_virtual`+`virtual_url`+`virtual_platform`) → `location_text` + `is_virtual` + `virtual_url`; hero image (`thumbnail_url`+`images[3]`) → `image_url`; type (`event_type`+`event_types[]`+`category`+`audience[]`) → `event_type`; network (`network`+`networks[]`+`network_only`+`network_ids`) → `network` (a label that drives the card badge only — no filtering/spoke attribution in the shell); recurrence (the 5-field mess) → `recurrence_rule` (ONE clean reserved column, NOT written by the shell); accessibility (`accessibility_features[]`+5 booleans) → `accessibility_features text[]` (ONE clean array, reserved). Status drops `pending_review` (`draft|published|cancelled`) — pending_review was load-bearing only for the out-of-scope auto-publish/network-tier gate; an owner publishes directly. recurrence_rule + accessibility_features are reserved anchors, not half-built features (DEC-218) — present as the one canonical representation, with the writer deferred.
- **(c) ONE RSVP table.** `public.event_rsvps` (event_id, user_id, status going|cancelled, `unique(event_id,user_id)`) — the EventRSVP-vs-RSVP split bug evaporates. Party-size / household / Joy-Coin-reservation / check-in columns are reference (not built).
- **(d) Writes route through SECURITY DEFINER helpers — the Base44 `manageEvent` god-function as Plant 1's focused family.** `create_event` / `update_event` / `duplicate_event` (→ a fresh draft) / `cancel_event` (→ status cancelled) / `delete_event` (soft-delete) gate on `auth.uid()` + `is_business_admin_or_above`; `set_event_rsvp` (the only helper a non-admin calls) upserts the caller's OWN row keyed on the unique, gating on (event published OR caller-is-member) + accepts_rsvps, with `user_id` from `auth.uid()` (a user can only RSVP as themselves). authenticated holds SELECT only on both tables; every helper writes an `audit_log` row + carries the inline SAFETY clause (one write path per concern, DEC-146; SD function count 47 → 53).
- **(e) RLS — member-scoped read, NO anon.** `events`: members of the business see all (incl. drafts); any authenticated user sees PUBLISHED events (the member-scoped read that enables a non-owner RSVP). `event_rsvps`: the RSVPer sees their own; members of the event's business see all (the owner-sees-counts read). anon gets NOTHING on both (DEC-224/225 grant block — verified post-apply: anon `42501 permission denied` on both tables).
- **(f) The single hero image lives in a private `event-images` Storage bucket.** Mirrors the `branding` bucket (DEC-233 storage pattern): path `<business_id>/<uuid>`, `storage.objects` RLS keyed on the first path segment (read = is_business_member, write = is_business_admin_or_above); the event row stores the PATH, the owner surface resolves it to a short-lived signed URL at render. Private by default (DEC-203) — no public read.
- **(g) Scope boundary — SHELL ONLY.** NOT built (stays as reference in community-node, planted later): Joy Coins, dollar ticketing/checkout, recurring-series generation, household/party-size RSVP, network filters, spoke attribution, the public discovery page, and — critically — **the public anon-read path**. When the public page lands it needs the SAME column-scoped anon-read decision the Directory work flagged (Postgres RLS is row-level; an `anon SELECT` on `events` gated on `status='published'` would expose EVERY column of a published event). The decision is flagged inline in the migration footer + in SECURITY-PRACTICES, NOT opened here.

**Rationale:** Resurface-before-rebuild (DEC-173) made literal — the captured widget is the spec; Plant 1 re-skins its look + behavior but collapses its accreted divergences instead of inheriting them. One-write-path-per-concern (DEC-146) keeps the manageEvent god-function from re-forming as one untestable surface; the focused family mirrors the scheduler's create/update/delete/duplicate shape. The member-scoped-read-but-no-anon posture is the honest middle: the owner room is real now, RSVP works for authenticated members, and the public-read decision (the genuinely consequential one — it makes every column public) is deferred WITH the public page rather than smuggled in early. Two-World (DEC-203): the SD helpers are the bridge layer, the private bucket keeps an image seen only by deliberate render. The reserved recurrence_rule + accessibility_features columns honor DEC-218 — one clean canonical shape now, the Base44 mess never re-imported, the writer added when the feature is genuinely wanted.

**Companion to:** DEC-223 (rebuild — the capture is reference, not a source to copy), DEC-146 (one write path per concern; one shared `lib/events/events.ts` module), DEC-139 (server-authoritative identity — created_by/deleted_by/rsvp user_id from auth.uid()), DEC-203 (two-world — the SD bridge + the private bucket), DEC-209 (honest navigation — the city Events tile stays dark until its public surface exists), DEC-218 (no half-done — reserved columns, not half-built features), DEC-221 (ownership-FK convention — business_id + created_by_user_id), DEC-224 + DEC-225 (the grant block), DEC-227 (Hyphae builds/commits/pushes/applies; the walk is Doron's), DEC-228 (records co-evolve), DEC-233 (the storage bucket + RLS pattern this mirrors).

**Evidence:** locallane `fe44324` on `feat/events-shell` (migration `20260608120000_create_events.sql` + `lib/events/events.ts` + the `events/` room — `page.tsx` + 3 `_actions` [event-actions, rsvp-actions, upload-event-image] + 2 `_components` [events-room, event-editor] — + the hub Events tile in `app/(authenticated)/businesses/[id]/page.tsx`). Migration applied to remote `thpxbguyegpzzxcwnuec` via `npx supabase db push` (dry-run → apply, "Applying… / Finished"). Verified: `npx tsc --noEmit` exit 0; `npx next lint` clean (no warnings or errors); `npx next build` clean (the `/businesses/[id]/events` route compiles, 9.97 kB); migration verified live via REST (`anon` → `42501 permission denied for table events` + `… event_rsvps`, confirming both tables exist AND the grant block closed anon). The owner round-trip (create → RSVP → count; the admin-gated SD writes + storage upload + signed-URL render) is Doron's authenticated walk — an owned business + the admin-gated RPCs can't be driven headlessly. Records: [[events]] (new, records/buildings); `SECURITY-PRACTICES-AUTH.md` "Events Shell — Tables RLS + SD Write Paths + event-images Storage RLS + Deferred Anon-Read"; STATUS-LEDGER Planted. Paired builder: `HYPHAE-BUILD-EVENTS-SHELL-COMPLETE`. Community-node mirror owed in the next focused drift-sync pass per DEC-182.

**Cross-references:** DEC-223, DEC-146, DEC-139, DEC-203, DEC-209, DEC-218, DEC-221, DEC-224, DEC-225, DEC-227, DEC-228, DEC-233.

---

### DEC-237: Assembler E-Sign Seal — No-Login Client Signing, the Signing-Boundary Lock (amends DEC-229), and the Anon Token-Scoped Surface (2026-06-08)

**Date:** 2026-06-08
**Status:** Active — **LIVING**. Built on `feat/signing` off `main` (locallane `c383ad2` + `ed21e59` + `f1754b1` + `bd3c99c` + `a7492bc`); migration `20260608180000` applied to the remote (apply-before-merge satisfied; the Finance ledger migration is local/unapplied — no collision); the full loop **walked clean on the preview** (send → email → no-login sign → seal/lock → convert files the signed doc to the client; EIN stripped); `feat/signing` **merged to `main` via `304641c`** (pushed; production deploy healthy — `/sign/[token]` live + public, home auth-gated). Paired planner: `MYCELIA-PLAN-SIGNING-READY-TO-BUILD`; the fork decisions are `MYCELIA-PLAN-SIGNING-AUDIT-FORKS`.

**Context:** DEC-234 made the estimate a branded client document + froze a snapshot on send; DEC-235 added the clean PDF. The last mile — deliver the document to the client, let them sign with NO account, and seal it — was the dormant 6.1 reserve (`signature_token` + `signed_at` on `fs_documents`, never set). A VERIFY-FIRST audit (this arc) ground-truthed: Resend greenfield; the acceptance-boundary lock (DEC-229) lived at FIVE sites keyed on `status in ('accepted','declined')`; `signature_token` is NOT NULL / auto-generated (the portal key already exists — no new column); `signed_at` is the dormant nullable col; no signer-evidence columns existed; and — a security finding — the EIN (`tax_id`) was leaking into `fs_documents.merge_data` via the send action's `...data` spread. Doron locked three forks: full seal, two-paths-to-a-project, full legal layer.

**Decision:**
- **(a) Signing IS the yes.** No separate client "accept" step — opening the no-login link + signing = acceptance + seal + lock, one motion. The estimate flips to status `'signed'` (the lock key); the `fs_documents` row hard-seals (`status='signed'` + `signed_at` + the signature bundle).
- **(b) The lock moves from the ACCEPTANCE boundary to the SIGNING boundary — this AMENDS DEC-229.** DEC-229's acceptance-boundary lock (the silent taxonomy-freeze + the send-guard, keyed on accepted/declined) is REPLACED: the seal fires on `'signed'` ALONE, and it is a FULL seal, not a taxonomy-only freeze. Draft / sent / viewed / accepted / declined are all SOFT now (editable, re-sendable, archivable); `'signed'` is immutable. `update_fs_estimate` raises on signed (whole-row immutability; the former accepted/declined taxonomy-freeze CASE is removed); `send_estimate_document` raises on signed (duplicate to revise); `delete_fs_estimate` raises on signed (a signed contract is undeletable — Doron's full-seal call); the two client mirrors (`document-controls.tsx` send-gate, `estimate-form.tsx` taxonomyFrozen + the new sealed-read-only banner) key off `'signed'`. `'declined'` drops out of the lock entirely.
- **(c) Two paths to a project.** `'accepted'` stays a SOFT manual "offline-yes" (a verbal/paper acceptance the owner records, still editable) that ALSO enables convert; `'signed'` is the e-signed hard seal. `convert_estimate_to_project` gains a SERVER-SIDE status gate: `status in ('accepted','signed')` — closing the former UI-only gate the audit found (a direct RPC could convert a draft). Convert stays MANUAL.
- **(d) One convert action, two outcomes.** Convert (off accepted OR signed) creates the project AND files the signed document under it (`fs_documents.project_id` back-link). The signed doc already carries `client_id` (stamped on send, DEC-213 fan-out), so it also lands in the client's Documents — convert completes the project context (project + client + estimate).
- **(e) The anon signer is gated by the TOKEN, not a login.** The no-login client has no `auth.uid()`; authorization is the unguessable `signature_token`. Two anon-EXECUTE SECURITY DEFINER functions are the ENTIRE anon-reachable Desk surface: `get_signing_document(uuid)` (returns only client-safe fields — the frozen EIN-free content HTML + presentation; NULL on a non-matching token) and `sign_estimate_document(uuid, jsonb)` (validates token + sent-status + UETA consent, computes the content SHA-256 SERVER-SIDE, hard-seals, flips the estimate, audit-logs with `actor_user_id` null). Anon gets NO table grant; each function acts only on the single row whose token matches — the inverse of the membership-gated owner helpers (e.g. `sign_fs_change_order` gates on `is_business_admin_or_above`).
- **(f) Full legal layer (UETA/ESIGN).** The client signs with a typed legal name + a drawn mark (pointer-event canvas → PNG data-URI) + date + an explicit UETA/ESIGN consent checkbox (the consent text — Oregon ORS 84.001 + federal ESIGN — stored verbatim). The server captures the IP (request headers) + a SHA-256 of the FROZEN content (tamper-evidence, computed over the stored bytes, never a client hash). The bundle (`signer_name`, `signature_image`, `signature_kind`, `signed_date`, `signer_ip`, `consent_agreed`, `consent_text`, `content_sha256`, `signed_at`) lives on `fs_documents.signature` — the legal record of record.
- **(g) EIN never crosses to the client surface.** `tax_id` is removed from the document data path (`lib/desk/client-document.ts` engine output + `client-document-source.ts` loader), so it can't reach client-visible content NOR the frozen `merge_data` the anon portal can read. The owner manages the EIN on the Profile identity card — a separate read.
- **(h) LocalLane sends; the business is the identity.** The client email sends from `documents@mail.locallane.app` (DNS-verified domain) with the BUSINESS name as from-name and `business_identity.email` as reply-to. `business_identity.email` is required-for-signing (reply-to AND the owner sign-notification target) — captured IN-FLOW if missing (the capture reads the full identity incl `tax_id` so the full-replace upsert preserves it; written back to `business_identity`, NEVER the signup email). The send carries the sealed PDF (the same PDFShift helper as the download, Living Feet) + the no-login sign link; the owner is emailed (service-role read of the private target) when the client signs.

**Rationale:** Two-World (DEC-203) at the seal layer — the signed document is the executed artifact crossing to the outside world; the token-capability + the EIN strip make the inside/outside membrane literal on the genuinely anon surface. Full-seal honors "a signed contract is the executed legal artifact" (immutable + undeletable; duplicate-to-revise is the one path). The signing-boundary supersedes the acceptance-boundary because "signing IS the yes" — acceptance is now the client's signature, not an owner flip; `'accepted'` survives as the weaker manual/offline path (two paths to a project). One-engine-every-consumer (DEC-146): the same render → preview / snapshot / PDF / portal, and one PDFShift helper for the download + the email. Activating the dormant `signature_token` (rather than minting a new key) honors the 6.1 substrate's foresight; the SHA-256 + UETA consent give the seal a court-of-law posture (PROJECT-BRAIN "defensible in both worlds").

**Companion to:** DEC-229 (AMENDED — acceptance-boundary lock becomes the signing-boundary seal), DEC-234 (the render + snapshot this completes), DEC-235 (the PDF this attaches), DEC-233 (the identity letterhead + the private EIN table), DEC-232 (the dials the render honors), DEC-213 (documents live where used — the signed doc's estimate/client/project fan-out), DEC-203 (two-world — the seal is the bridge, the membrane holds on the anon surface), DEC-146 (Living Feet — one render engine, one PDF helper, one email module), DEC-139 (server-authoritative — owner writes from auth.uid(); the anon path is token-authoritative by design), DEC-209 (honest navigation — the client Documents stub becomes a real list), DEC-218 (additive; the dormant reserve activated, no half-built feature), DEC-224 + DEC-225 (no table-grant change; the two signing functions add anon EXECUTE on themselves only), DEC-227 (Hyphae builds/commits/pushes/merges; the apply + walk are Doron's — gated here), DEC-228 (paired records co-evolve).

**Evidence:** locallane `feat/signing` — `c383ad2` (migration `20260608180000_estimate_signing_seal.sql`: `fs_documents.signature` jsonb; `get_signing_document` + `sign_estimate_document` anon-token SD functions; lock moved to signed across send/update/delete/convert + convert files the doc; EIN strip) + `ed21e59` (UI lock mirrors + 'signed' badge) + `f1754b1` (the `app/sign/[token]` portal + signature capture + sign action + owner notification + Resend infra + `lib/desk/pdf.ts` + ClientDocument executed-aware) + `bd3c99c` (Send emails the proposal + in-flow business-email capture) + `a7492bc` (client Documents section + sealed read-only treatment). Verified: `npx tsc --noEmit` exit 0; `npx next lint` clean; `npx next build` clean (`/sign/[token]` compiles, 2.47 kB) — on the branch AND on merged `main`. Migration applied to the remote; merged to `main` via `304641c` (pushed); production deploy healthy. The full loop walked clean on the preview (send → no-login sign → seal → estimate-tab signed → owner email → convert → doc in client Documents). Records: [[assembler_client_document]] change log + [[desk_estimates]] + [[desk_people]] change logs + `SECURITY-PRACTICES-DESK.md` "Assembler E-Sign Seal — No-Login Portal + Anon Token-Scoped Write Path + EIN Redaction" + STATUS-LEDGER Planted. Paired builder: `HYPHAE-BUILD-SIGNING-SEAL-COMPLETE`. Community-node DECISIONS mirror (DEC-237) owed in the next focused drift-sync per DEC-182.

**Cross-references:** DEC-229, DEC-234, DEC-235, DEC-233, DEC-232, DEC-213, DEC-203, DEC-146, DEC-139, DEC-209, DEC-218, DEC-224, DEC-225, DEC-227, DEC-228.

---

**DEC-237 — Amendment (2026-06-09, Ground-True session): the sign-time drift check — the seal refuses a superseded snapshot, and the signed numbers are the source of record.**
The Desk builder's walk (2026-06-09) verified one major hole in the seal as ratified: a `'sent'` estimate stays editable (decision (b) made sent SOFT by design), the client signs the FROZEN `fs_documents` snapshot, and `sign_estimate_document` flipped the LIVE `fs_estimates` row to `'signed'` with **no check that the live row still matched the snapshot** — the seal could stamp a drifted estimate, and `convert_estimate_to_project` then read the post-edit live total into `original_budget` while the legal artifact said otherwise. Live legal behavior; repaired at the boundary:

- **The seal refuses a superseded snapshot.** `sign_estimate_document` raises (`superseded`) when the live estimate was edited after the snapshot was sent — predicate `fs_estimates.updated_at > fs_documents.updated_at`; both are stamped by the same transaction clock inside `send_estimate_document`, so strictly-greater means exactly "edited after send" — or when the estimate was soft-deleted since. The estimate row is locked `FOR UPDATE` through the seal (doc-then-estimate, the send path's lock order) so a concurrent owner edit cannot race the check. Self-healing: re-sending re-freezes the snapshot and clears the condition; the client's link stays the same.
- **The portal says it kindly, early.** `get_signing_document` returns a `superseded` boolean (same predicate); the portal renders "this document was updated after it was sent — ask for a fresh copy" in place of the signing panel at page load, and the sign action maps the race-window raise to the same friendly copy. The seal-time guard is the load-bearing boundary; the load-time read is courtesy.
- **Convert takes the SIGNED numbers.** `convert_estimate_to_project` on a `'signed'` estimate reads `original_budget` from the signed document's frozen `merge_data->'totals'->>'total'`, not the live row — the executed legal artifact is the source of record (it also covers any document sealed before this guard existed). `'accepted'` (the soft offline-yes, no snapshot) keeps the live total — on that path the live row IS the agreed number. The audit payload names the `budget_source`. The alternative of re-deriving the whole project from the snapshot was considered hands-in-the-code and rejected for v1: the snapshot's frozen totals carry the agreed number; full re-derivation belongs to the future estimate→project line-set work, not a drift repair.

Evidence: locallane `02e12c4` on `feat/ground-true`, merged to `main` via `e87abef`; migration `20260609190000_sign_time_drift_check.sql` applied to remote (dry-run → apply, clean) + probed (null on bad token / invalid-link raise / grants intact); tsc + lint + build + the new vitest net clean. Records: SECURITY-PRACTICES-DESK seal entry updated (guard #5); STATUS-LEDGER Last-touched. Paired builder: `HYPHAE-BUILD-GROUND-TRUE-COMPLETE`.

---

---

### DEC-238: Finance Ledger Spine — the Double-Entry Money-Record Layer (Accounts · Transactions · Entries), Integer-Cents + Append-Only + Idempotent; Pool/Stripe/Stored-Value Excluded (2026-06-08)

> **Numbering note (2026-06-09, Ground-True session).** This decision was
> ratified on `feat/finance-ledger` (Spec-Repo branch close-out `d42aece`)
> citing **DEC-237** — a number main had concurrently assigned to the e-sign
> seal (the two close-outs ran on parallel tracks the same day; the DEC-100
> double-booking lesson repeating). Renumbered here to **DEC-238**, the next
> free number stamped at the source, and the body landed on `main` so the
> index can never double-book it again. The build branch was rebased +
> renumbered + re-stamped to match and stays **UNMERGED**.

**Date:** 2026-06-08 (ratified on-branch) · renumbered + reconciled 2026-06-09 · **merged + applied + sealed 2026-06-10 (DEC-241)**
**Status:** Active — **ON PRODUCTION.** Updated 2026-06-10 (Growth-Queue Session 3, "The Books"): `feat/finance-ledger` rebased onto the identity seam and MERGED to `main` (locallane `6ec4ac8` + door repair `a766fb2`); migration `20260609210000_finance_ledger_spine.sql` APPLIED to the remote (out-of-order vs the applied seam — `npx supabase db push --include-all`, the CLI's prescribed path) and the first real poster has written: the fs_payments bridge backfill posted the $900 client payment as `fs_payment:<id>:v1` (2 legs, balanced, audited). The spine was SEALED before merge per DEC-241 (posting door, identity pin, structural reversals, bank-notch bones, seam-shaped walls). Planted → Living at Doron's walk. Paired planner: `MYCELIA-PLAN-FINANCE-LEDGER-SPINE` (build) + `MYCELIA-PLAN-FINANCE-OFF-THE-BENCH` (the merge/seal/bridge session). Paired builder: `HYPHAE-BUILD-FINANCE-LEDGER-SPINE-COMPLETE` (build) + `HYPHAE-BUILD-FINANCE-OFF-THE-BENCH-COMPLETE` (Session 3). *(Original hold-state status, superseded:* Ratified at the Finance ledger-spine build on branch `feat/finance-ledger` (locallane `1b38987` after the 2026-06-09 reconciliation rebase — NOT merged to `main`; staged behind the Construction Gate); migration committed but NOT applied — money behind the gate. Planted → Living when Doron authorizes the apply AND the first real poster writes a balanced transaction.*)*

**Context:** Finance is the spine of the Assembler + Events touch (invoices and ticket sales represent money) per the master seam map. The authoritative spec is `MYCELIA-RESEARCH-FINANCE-SPINE` (the four disciplines, three objects, the Stripe line, the traps); the scope boundary is `MYCELIA-PLAN-FINANCE-POOL-SEPARATION` (pool is lawyer-gated). A VERIFY-FIRST audit established the field before any code: Plant 1 had **no double-entry ledger** — only `fs_payments`, a single-entry per-project manual payment LOG (its comment calls itself a "ledger" loosely) storing money as `numeric(14,2)`; **no Stripe in code** (no SDK/keys/Connect/webhook — only future-tense comments; no `business_stripe_account` table despite CLAUDE.md's aspirational "reserved" — that CLAUDE.md fiction was itself corrected on `main` at the Ground-True session); **no Finance tile/route** (`lib/desk/financials.ts` is the per-project rollup); money everywhere is `numeric(14,2)` exact decimal — never float, but never integer-cents either.

**Decision:**
- **(a) Build the double-entry RECORD LAYER ONLY — three objects.** `ledger_accounts` (a pool of value: `account_type` asset|liability|equity|revenue|expense + `normal_balance` CHECK-pinned to type + `is_clearing` ⇒ asset for money mid-flight; `business_id` NULL = platform-scope, SET = that business's own account — **individual owed-balances, never one aggregate**, per the research), `ledger_transactions` (groups ≥2 balanced entries; `idempotency_key` NOT NULL UNIQUE; `source`; `reverses_transaction_id`), `ledger_entries` (one immutable movement: `direction` debit|credit + `amount_cents bigint` > 0). Plus a `security_invoker` `ledger_account_balances` view (balances COMPUTED, never stored) and two SECURITY DEFINER helpers (`create_ledger_account`, `post_ledger_transaction`), `service_role`-only.
- **(b) The four disciplines, hard-enforced at the DB layer.** **(1) Integer minor units** — `amount_cents bigint`; no numeric/float for money anywhere (the `numeric(14,2)` Desk world converts ×100 at a deferred bridge; bigint not integer for platform all-time headroom; Stripe-native). **(2) Double-entry** — TWO deferred constraint triggers (on `ledger_entries` AND `ledger_transactions`) calling one shared assertion: at COMMIT every transaction has ≥2 legs and nets to zero per currency. Guarding both tables closes the zero-leg-transaction bypass (a direct `service_role` insert of a leg-less txn is caught by the transactions-side trigger). **(3) Append-only immutability** — `BEFORE UPDATE/DELETE/TRUNCATE` triggers raise for EVERY role (incl. `service_role` + the SD owner; triggers aren't bypassed by BYPASSRLS); corrections are reversing transactions; **every FK is `ON DELETE RESTRICT`** (no SET NULL/SET DEFAULT — an RI-driven UPDATE would be blocked by the trigger and wedge the parent delete). **(4) Idempotency** — `idempotency_key` NOT NULL UNIQUE; the post helper returns the existing txn on a key hit and re-raises any OTHER unique violation (never a silent NULL); the guarantee is KEY-scoped (callers supply a stable, event-deterministic key).
- **(c) Posting is `service_role`-only.** `post_ledger_transaction` + `create_ledger_account` are SECURITY DEFINER, EXECUTE revoked from public, granted to `service_role` ONLY (not authenticated) — posting to the truth-ledger is a trust-boundary/system act (the future Stripe webhook, internal flows), never a user write. The post helper validates each leg's account (exists/active/currency-match), inserts txn+legs atomically, runs an early per-currency balance check, writes a `finance.ledger.post` audit_log row (amounts + leg count; no account-holder PII).
- **(d) RLS member-scoped; platform books private; a deliberate grant deviation.** `authenticated` holds SELECT only, scoped to its own business's rows (`is_business_member`); platform-scope rows (`business_id` NULL) are `service_role`-only in v1. `service_role` gets full DML on `ledger_accounts` but **SELECT + INSERT only on the two immutable tables** — a deliberate deviation from DEC-225's full-DML default: withholding UPDATE/DELETE IS the immutability guarantee, reinforcing the trigger.
- **(e) Scope boundary — RECORD LAYER ONLY.** It moves no real money and is wired to nothing. **NOT built (room left, nothing built):** the community/revenue **POOL** (lawyer-gated — counsel before its collect-and-redistribute structure; posts through the same API into future liability/equity accounts when it exists), live **STRIPE rails** (the future webhook posts via the seeded `stripe_clearing` account with `source='stripe.charge'`, `idempotency_key=<event_id>` — no SDK/charge/transfer code), any **WALLET / stored-value / coin balance** (licensing — never; balances are derived, never stored), and the **`fs_payments`→ledger bridge** (it would touch the live Desk track this parallel build must not).

**Rationale:** The research's NOW tier is exactly integer-cents + an immutable double-entry ledger; this builds that and nothing past it. Enforcing the four disciplines at the DB layer (not the app) means the truth holds regardless of who posts — the SD helper, a direct `service_role` insert, or a future webhook. The `numeric(14,2)`→cents divergence is deliberate and spec-driven (discipline #1), with the conversion seam flagged rather than smuggled. Posting-as-`service_role` follows directly from "Stripe webhook posts; UI never mutates a posted entry" (DEC-203 two-world — the ledger is the inside-truth, Stripe is the bridge). The immutable-table grant withholding is the rare case where DEC-225's "withholding is friction without security" is FALSE — here withholding IS the security. No stored value keeps Plant 1 structurally clear of the Oregon money-transmitter line.

**Companion to:** DEC-203 (two-world — the inside-truth ledger, the Stripe bridge, no stored value), DEC-224 + DEC-225 (the grant block + the deliberate immutable-table deviation), DEC-146 (Living Feet — one posting path, one balance assertion, one view), DEC-139 (server-authoritative — posting is a trust-boundary act), DEC-167 (schema-conformance), DEC-223 (rebuild — the community-node "Finance node"/FinancialProfile did NOT come over; this is fresh), DEC-227 (Hyphae builds/commits; the apply + walk are gated), DEC-228 (records co-evolve). Parallel to DEC-236 (Events shell — the sibling track on its own branch; no shared table/column/function/route). Numbering collision with DEC-237 resolved at the Ground-True session (2026-06-09).

**Evidence:** locallane `1b38987` on `feat/finance-ledger`, pushed to origin for safekeeping (migration `20260609210000_finance_ledger_spine.sql` — 3 tables + 1 view + 2 SD helpers + 3 trigger/assert functions + 4 immutability triggers + 2 deferred balance constraint triggers + 4 seeded platform accounts; renumbered DEC-237→DEC-238 across all 10 in-file citations incl. the runtime immutability RAISE message). NOT merged; NOT applied to remote (deliberate hold — see Status). Verification at the original build: a 5-lens adversarial review before commit (double-entry, immutability, idempotency/concurrency, Postgres/RLS/conventions, scope/seams) — two blockers found + fixed pre-commit (zero-leg dangling transaction → the transactions-side deferred trigger; `created_by_user_id ON DELETE SET NULL` colliding with the immutability trigger to wedge user deletion → `ON DELETE RESTRICT` + the no-SET-NULL invariant), plus the idempotency exception-swallow (now re-raises), currency case-divergence (now lowercase-normalized), and per-leg account validation; structural checks (balanced dollar-quotes; zero numeric/float money columns; zero SET NULL/SET DEFAULT FKs). Records: [[finance_ledger]] (records/systems, on the records branch `feat/finance-ledger` until merge); `SECURITY-PRACTICES-AUTH.md` "Finance Ledger Spine" + roster line (same branch); STATUS-LEDGER Planted entry (same branch). Paired builder: `HYPHAE-BUILD-FINANCE-LEDGER-SPINE-COMPLETE`. Community-node DECISIONS mirror (DEC-238) owed in the next focused drift-sync pass per DEC-182.

**Cross-references:** DEC-203, DEC-224, DEC-225, DEC-146, DEC-139, DEC-167, DEC-223, DEC-227, DEC-228, DEC-236, DEC-237 (the number it briefly borrowed).

---

---

### DEC-239: Every Person Is All Things — One First-Class Person; Business, Worker, Client Are Relationships, Each Personal to Each Relationship (2026-06-09)

**Date:** 2026-06-09
**Status:** Active — **RECORDED, NOT YET BUILT.** This session (Ground-True, Growth-Queue Session 1) only ledgers the decision; Growth-Queue Session 2 (the identity seam) builds it. Paired planner: `MYCELIA-PLAN-GROUND-TRUE-MICRO-SESSION` (the recording) + the Session-2 plan (the build).

**Context:** REBUILD-SCOPE §13 **Q5** — the single biggest open schema decision since Session 1 (2026-05-23): does Plant 1 ship Approach A (`workers_json`, the Base44-era Phase-2 shape) or Approach B (a first-class person entity)? The five-walk synthesis (2026-06-09) put the identity seam first in the growth map, and Doron resolved the question above both approaches.

**Decision (Doron, verbatim):**

> *"Every person should be all things; a business, a person, a worker. Each relationship is personal to each relationship."*

Operationally: there is **ONE first-class person**. Business, worker, and client are **relationships a person holds** — never separate identity types, never a JSON blob on someone else's row. Each relationship record carries its own context, scope, and visibility — *personal to each relationship*: what a person shares as a worker on one crew is not what they share as a client of another business, and neither leaks into the other. This resolves Q5 as **first-class people + relationship records** — a generalization of Approach B (Approach B made the worker first-class; this makes the PERSON first-class and demotes worker/client/owner to relationship rows). `workers_json` (Approach A) is rejected for the rebuild: free-text crew is already a verified walk-finding-class limitation (one-arg predicates can't scope per-project crew; crew as free text), and a JSON shape would freeze it.

**Rationale:** The organism principle made structural — "the organism is the relationship in between" (DEC-123): people are nodes, relationships are the hyphae, and each hypha carries its own contract of visibility (DEC-203 two-world at the person scale; DEC-232's dial-per-section pattern at the relationship scale). One person table also keeps the future federation arc walkable: a person is one being across every business they touch, with per-relationship scope instead of duplicate identities.

**What this is NOT (yet):** not a schema, not a migration, not a People-room rebuild. Session 2 answers the Five Questions, designs the person + relationship tables against the existing `business_users`/`fs_clients`/`assignee_name` ground, and ships it per BUILD-PROTOCOL.

**Origin:** Doron surfaced this as the King's decision at the five-walk synthesis (2026-06-09); recorded verbatim by Hyphae at the Ground-True session per the planner entry's instruction. Numbers stamped at the source (DECISIONS.md), per the DEC-237/DEC-238 double-booking lesson.

**Companion to:** DEC-123 (the organism is the relationship), DEC-203 (two-world — per-relationship visibility), DEC-221 (ownership-FK convention the relationship rows will follow), DEC-232 (dial-per-section — the visibility grammar relationships inherit), DEC-227 (build gated to Session 2).

**Cross-references:** DEC-123, DEC-203, DEC-221, DEC-232, DEC-227, DEC-228. REBUILD-SCOPE §13 Q5 (resolved by this DEC).

---

---

### DEC-240: The Identity Seam — First-Class People + Relationship Records (builds DEC-239); Two-Arg Crew Predicates + Crew Write Tier Before the Finance Merge; the NEVER Lock on the Signing Key (2026-06-09)

**Date:** 2026-06-09
**Status:** Active — BUILT AND ON PRODUCTION. locallane `main` `c1313a8` (merge of feat/identity-seam `138b5de`); migration `20260609220000_identity_seam.sql` applied to remote + probed. Planted → Living at Doron's walk (People room → add a worker → dials → assign to a project). Paired planner: `MYCELIA-PLAN-IDENTITY-SEAM`.

**Context:** DEC-239 ratified the principle ("Every person should be all things; a business, a person, a worker. Each relationship is personal to each relationship") and gated the build to Growth-Queue Session 2. The Desk walk (2026-06-09) had three open moderates this build closes: crew identity accumulating as free text; one-arg predicates unable to express per-project crew scope (the seam had to be cut BEFORE the finance branch merges, so the ledger's walls inherit a wall-shape that can scope per-project); and `fs_documents` member SELECT exposing `signature_token` (forge capability the moment a non-owner member tier exists — which this same session creates).

**Decision (the seam, as built):**
- **(a) One first-class person.** `public.people` — platform-level, NOT fs_-scoped (a person is one being across every business they touch; the federation arc stays walkable). Tier-2 posture mirrors `fs_clients`: the creating business holds the pen (`created_by_business_id`, ON DELETE RESTRICT); no name/email uniqueness — identity CONVERGES at claim time, when one auth user claims several person rows via `linked_user_id` (FK now, per the 6.3.1 forward note "the FK can land later when workers/subs/vendors-with-linked_user_id consolidates" — this was that consolidation). The Tier-3 pen inversion (the `update_fs_client` 20260528233000 lock) is mirrored in `update_crew_member`: once claimed, the person's identity fields are theirs; the relationship half stays the business's side.
- **(b) Relationship records carry the scope.** `public.person_relationships` (person × business × kind `worker|sub|vendor`): status, title, default_rate, notes, and a per-relationship `visibility` dial object — "personal to each relationship" made literal. `fs_project_crew` is the project dimension. Owner-side management is the People room (Workers/Subs/Vendors live); pickers replace the free-text `worker_name`/`assignee_name` capture (display names stay as snapshots per DEC-218; new structured columns `worker_relationship_id`/`assignee_relationship_id`).
- **(c) The two-arg predicates.** `fs_crew_can_read(business, project, section)` / `fs_crew_can_write(business, project, section)` ask the real question — THIS person (auth.uid() → claimed relationship), THIS business, THIS project, THIS section — resolving the DEC-232 three-layer dial stack (SHIP default → `default_crew_visibility[kind]` → relationship override; SQL `fs_crew_section_on` + TS `getCrewVisibility`, vocabularies pinned by tests). **Crew NEVER get `business_users` rows** (a member row inherits blanket fs_* reads — the opposite of money-off), and **the fs_* member SELECT policies never grow crew branches** (money rides on the same rows as field data; row-level policies cannot column-mask). The crew READ surface, when it lands, is section-scoped SD RPCs — the signing-portal shape, the established pattern for non-member audiences. The finance spine (DEC-238, still unmerged) adopts THESE predicates at Session 3; no new one-arg shapes.
- **(d) The crew write tier.** `create_fs_labor_entry` gains the crew branch: a claimed, active, ASSIGNED relationship with the `daily_logs` dial on may log its own hours — identity and money forced from the RELATIONSHIP server-side (worker pinned to the caller's own relationship; rate from `default_rate`; payload money ignored; hours bounded 0–24). Write tier hard-fenced to field sections (`schedule`/`daily_logs`/`photos`) — a crew hand never writes money regardless of dials.
- **(e) Safe defaults.** Field sections ship ON (schedule, daily_logs, photos); money sections ship OFF (documents, materials, labor_costs, payments, financials) — DEC-220 lineage. Everything dormant until a person claims an account; a solo owner notices nothing (empty roster renders yesterday's free-text inputs verbatim).
- **(f) The NEVER class.** `fs_documents.signature_token` is column-revoked from `authenticated` (the re-grant lists every column but the key); the one authenticated door is `get_document_signing_token()` — admin-gated, audited per access. The send flow never emails without a working sign link. Some things aren't toggles, they're locks — same row as "Internal notes — never" (DEC-232c), now joined by "Client signing keys — never". Prophylactic BY DESIGN: the member set equals the owner set today, and the lock landed in the same migration that makes non-owner tiers possible.
- **(g) What this deliberately is NOT.** `fs_clients` flows untouched — fs_clients IS the client-relationship record (subsume-not-parallel); it gains only the `person_id` seam, backfilled when a client-arc session rebuilds those flows. The claim flow (invitations `fs_person` acceptance), the worker's own surfaces (day view, time clock), and the estimate Permissions Workers/Subs/Vendors tabs (estimate-DOCUMENT dials, a different vocabulary) all stay COMING — they land with crew onboarding. The finance branch was not merged or touched.

**Rationale:** DEC-239's principle made structural at the cheapest possible moment — before free text accumulates further and before the finance walls calcify one-arg. The write-enforcement ground (authenticated holds SELECT-only on fs_* tables; the RLS write policies are dormant; SD helpers are the real gate) dictated that the crew write tier live in helper Layer-2 gates, not new RLS write policies. The money-on-field-rows ground dictated that crew reads be RPC-shaped, not policy branches.

**Evidence:** locallane `138b5de` (the build) merged via `c1313a8`; migration `20260609220000` applied via `npx supabase db push` (dry-run → apply clean). Verified: tsc exit 0; `next lint` clean (2 pre-existing warnings); `next build` clean; vitest **41/41** (12 new crew-dial tests); CI green on branch + main. Live probes: anon 42501 on people/person_relationships/fs_project_crew + both crew RPCs; **authenticated probe user (created + deleted in-probe): granted fs_documents columns 200, `signature_token` 42501, `select=*` 42501** — the NEVER lock holds live. Pre-commit adversarial review (4 lenses + adversarial verification, 10 agents): 1 blocker + 5 majors found and fixed before commit (CrewPicker custom-state, editor re-seed on expand, PhoneInput conformance, crew-hours bounding, appointment-pointer clearing on removal, send-flow token-error guard) + 11 minors fixed.

**Companion to:** DEC-239 (the principle), DEC-232 (the dial grammar extended per audience), DEC-220 (workers never money — now structural), DEC-218 (free text stays valid; nothing dropped), DEC-227 (Hyphae applies; Doron walks), DEC-237 (the signing key the NEVER class protects), DEC-238 (the finance spine that adopts the seam at Session 3), DEC-221 (ownership-FK convention), DEC-224/225 (grants discipline).

**Cross-references:** DEC-218, DEC-220, DEC-221, DEC-224, DEC-225, DEC-227, DEC-228, DEC-232, DEC-237, DEC-238, DEC-239. Record: `context/records/systems/people_relationships.md`. Security: `SECURITY-PRACTICES-AUTH.md` "Identity Seam" entry.

---

---

### DEC-241: The Books Open — the Spine Sealed (Posting Door + Identity Pin + Structural Reversals), Walls in the Seam's Shape, Bank-Notch Bones, Driver-Out Drawn, and the fs_payments→Ledger Bridge (Pen, Not Pencil) (2026-06-10)

**Date:** 2026-06-10 (Growth-Queue Session 3, run 2026-06-09 → 10)
**Status:** Active — **ON PRODUCTION.** locallane `main` `a766fb2` (merge `6ec4ac8` + door repair); migrations `20260609210000` (the sealed spine) + `20260609230000` (the bridge) applied to remote via `npx supabase db push --include-all` and probed live (six refusal probes + anon/authenticated wall probes, all holding; backfill verified balanced + audited). Planted → Living at Doron's walk. Paired planner: `MYCELIA-PLAN-FINANCE-OFF-THE-BENCH` (+ `MYCELIA-PLAN-BANK-SYNC-VISION` for the notch bones). Paired builder: `HYPHAE-BUILD-FINANCE-OFF-THE-BENCH-COMPLETE`.

**Context:** DEC-238 planted the double-entry spine on its bench branch; DEC-240 cut the identity seam first so the money walls would inherit a wall-shape that can scope per-project. The walls audit (`HYPHAE-AUDIT-FINANCE-WALLS-2026-06-09`) found two cracks where the spine's comments promised what its triggers did not enforce: a balanced PAIR of legs could be APPENDED to an already-posted transaction (the deferred balance check judges the whole set, so a balanced append passes), and `ledger_accounts` identity columns stayed fully mutable to service_role while every balance is DERIVED from entries against them. This session merges the spine, seals the cracks as locks, and wires the first real money in.

**Decision:**
- **(a) Crack A sealed — THE POSTING DOOR.** `post_ledger_transaction` generates the transaction id FIRST, opens a transaction-local door (`set_config('locallane.ledger_door', <id>, true)`), inserts header + legs, and closes the door on every return path. `BEFORE INSERT` door triggers on BOTH truth tables admit a row only while the door names exactly its transaction id — the helper is structurally the ONLY door, and **a posted transaction locks WHOLE**: no leg-append (even a balanced pair), no header forging, for every role including service_role. Two single-table trigger functions by necessity — PL/pgSQL resolves record fields against the actual row type even in an untaken CASE branch (found live at the apply; see Evidence).
- **(b) Crack B sealed — THE IDENTITY PIN.** Once any entry references a `ledger_accounts` row, its identity columns (business_id, code, account_type, normal_balance, is_clearing, currency) raise on UPDATE; name / is_active / metadata stay mutable forever. The post helper takes `FOR SHARE` on each leg's account row, closing the first-post race (a concurrent identity change blocks, then the pin sees the new entries and refuses it).
- **(c) Reversals become STRUCTURAL, not conventional.** A transaction declaring `reverses_transaction_id` must exactly offset its target — per-account, per-currency net of the pair must be zero — enforced inside the only door (a forged "reversal" could otherwise mark a live posting reversed and desynchronize the bridge). A reversal may post onto a RETIRED (`is_active=false`) account; new money may not — the correction pen always works (an account retirement can never wedge a payment edit/soft-delete).
- **(d) The walls adopt the seam (DEC-240), as a LOCK.** The books are MEMBER-tier only (`is_business_member` SELECT policies). Crew hold no `business_users` rows, so they are structurally excluded — and these policies NEVER grow crew branches. If a future session ever dials crew toward money, it arrives as section-scoped SECURITY DEFINER RPCs gated by `fs_crew_can_read(business, project, 'financials')` — the signing-portal shape, the predicates cut for exactly this door. The books stay behind glass. Verified live: an authenticated non-member (policy-identical to crew) reads `[]` on all three tables + the balances view; anon gets 42501; the post helper is 42501 to authenticated.
- **(e) BANK-NOTCH BONES (no sync built).** Every money record carries its matched-to-the-bank state from birth: `ledger_transactions.reconciliation_status` (`unmatched` default | `matched`) + `bank_line_key` (nullable, partial-unique — one bank line witnesses at most one book record; key ⇒ matched CHECK). These two columns are the ONLY mutable ink on the table — a dedicated UPDATE guard (jsonb-minus comparison, future-proof against later columns) + a column-scoped `GRANT UPDATE (reconciliation_status, bank_line_key)` to service_role. The bank is the witness; the books are the story; a bank line with no book record is the outside-the-books boundary and lives in the future import table, never as a hole here. One bank line, one truth per purchase — the structural end of the double-count class.
- **(f) DRIVER-OUT DRAWN, not built.** The chart leaves room to someday pay a PERSON (a DEC-240 `person_relationships` worker/sub): a business-scoped LIABILITY account per payee (code convention `person_payable:<relationship_id>`, metadata carrying the relationship pointer), posted through the same door — no new account_type, no schema change. Nothing person-payable exists today.
- **(g) THE BRIDGE — fs_payments post into the books on settlement. Pen, not pencil.** A reconciler (`ledger_sync_fs_payment`), fired by an AFTER INSERT OR UPDATE trigger on `fs_payments`, converges each row: a settled (`received|cleared`), live (deleted_at NULL) payment has exactly ONE active posting — key `fs_payment:<id>:v<n>`, integer cents (`round(amount × 100)`, exact from numeric(14,2)), `received` → debit cash / credit job_revenue, `paid` → debit job_costs / credit cash, business_id off the row, occurred_at = the payment date (midnight UTC, GUC-independent). A money edit (amount/direction/business/date) posts a mirror-leg REVERSAL (`…:v<n>:rev`, `reverses_transaction_id` set) and a REPOST at v<n+1> — **the missing fix-a-mistake tool is born as a reversing entry**; un-settling or soft-deleting reverses without repost; deterministic keys make every action idempotent; same-row writes serialize on the row lock. The v1 business chart (cash · job_revenue · job_costs — individual accounts per business, never one aggregate) is created on first need through `create_ledger_account` (one creation path). Existing settled payments BACKFILLED through the same reconciler (1 payment — the $900 client payment — now in the books). Hard DELETE/TRUNCATE on fs_payments raise for every role (+ DELETE grant revoked from service_role); `fs_payments.id` is pinned (the books anchor to it). `update_fs_payment` re-issued: `payment_date` now PRESERVED BY OMISSION (it was reset-to-today on omitted payloads — pre-bridge a field blip, post-bridge permanent reversal churn). `post_ledger_transaction` gains an optional `metadata` param (provenance, not truth — the reconciler's identity comparison and the future Stripe webhook's event pointers).
- **(h) RETENTION — rule named, nothing built.** Nothing in Plant 1 represents retention/holdback (audited: no column, no code, none in the Base44 reference). When it lands it follows the same pen rule: held-back money is its own posted movement (a liability leg — owed, not yet moved); releasing it is another posting; never a silent edit.
- **(i) SPENT DOUBLE-COUNT — the builder's call: a NOTE, not a schema.** A dollar can enter the Desk through two doors (a material/labor entry AND a paid payment) and the Spent tile sums both. THE BOOKS ARE STRUCTURALLY CLEAN of this — only fs_payments post. The Desk-tile dedupe needs a link UI (the Finance-room session's design call); the structural end-state is the bank notch (e). A dedupe seam with no consumer would be over-building. Noted at the computation (`lib/desk/financials.ts`), in DEFERRED-ITEMS, and here.

**Rationale:** The walls audit's cracks were promises in comments; money infrastructure keeps its promises in triggers (the NEVER-lock lesson of DEC-240 applied to the ledger). The door turns "the post helper is the only door" from convention into mechanism, which then makes reversal-mirror enforcement COMPLETE (checking inside the only door covers every path) — each seal strengthens the next. The bridge is a reconciler rather than an event log so that every path (helper, future helpers, backfill, un-delete) converges to the same truth idempotently, and the Desk keeps its working-log ergonomics (DEC-214's mutable payments) while the books stay pen. Bones-not-sync for the bank notch and drawn-not-built for driver-out honor the don't-over-build line; the TS mirror (`lib/finance/ledger.ts`, pinned by 16 tests) keeps the conversion seam in one vocabulary across both halves (the DEC-240 crew.ts precedent).

**Companion to:** DEC-238 (the spine this seals and ships), DEC-240 (the wall-shape adopted; the predicates the future crew-money door uses), DEC-203 (two-world — the books are the inside-truth), DEC-146 (one posting path, one creation path, one conversion vocabulary), DEC-139 (the bridge is system-derived; actor NULL, the human act audited on the payment side), DEC-214 (payment edit capability — now reconciled to the books by reversal), DEC-224/225 (grants; the column-scoped notch UPDATE joins the immutable-table deviation), DEC-227 (Hyphae applies via `npx supabase db push`; Doron walks), DEC-228 (records + SuperMemory co-evolve).

**Cross-references:** DEC-139, DEC-146, DEC-203, DEC-214, DEC-218, DEC-224, DEC-225, DEC-227, DEC-228, DEC-238, DEC-239, DEC-240. Record: `context/records/systems/finance_ledger.md`. Security: `SECURITY-PRACTICES-AUTH.md` "Finance Ledger Spine" entry (updated) + "The Books Bridge" entry.

**Evidence:** locallane `79304e0` (the seal + bridge build) + `b1e7564` (door repair) on `feat/finance-ledger`, merged via `6ec4ac8` + `a766fb2`, pushed; CI green ×4 (branch + main); production healthy. Apply: dry-run flagged the out-of-order stamp exactly as anticipated; `--include-all` was the CLI's prescribed repair; the spine applied clean; the bridge's FIRST attempt failed at the backfill — `record "new" has no field "transaction_id"` (42703): the shared door function read a sister-table field in an untaken CASE branch, and since the spine itself posts nothing, the door first FIRED at the first real post. Repaired by my judgment as two single-table door functions — fixed at the source in the spine file (for any fresh database) AND converged on the already-applied remote via the bridge's §0 (idempotent both ways); second apply clean, backfill notice `1 settled payment(s) reconciled into the books`. Verified: vitest 57/57 (16 new ledger tests); tsc exit 0; `next lint` clean (1 pre-existing warning); `next build` clean. Live probes (Management API + REST): leg-append REFUSED (door, legs half), header forge REFUSED (door, header half), money-column UPDATE REFUSED (notch-only guard), identity pin HOLDS, fs_payments hard DELETE REFUSED, forged non-mirror reversal REFUSED; anon 42501 on all three tables + view; authenticated non-member (created + deleted in-probe; policy-identical to crew) reads `[]` everywhere and 42501 on the post helper; residue pristine (1 txn, 2 legs, 1 payment, 1 audit post, 0 probe users). Books state: cash 90000 debit / job_revenue 90000 credit, notch `unmatched`, `bank_line_key` NULL. Pre-merge adversarial review (two rounds, 5 lenses + per-finding adversarial verification, 35 agents): 7 confirmed findings fixed pre-merge (inactive-account reversal wedge; unenforced reversal provenance → the mirror seal; fixed-epsilon cents guard; DateStyle-dependent date identity; NULL-NOT-IN account-type guard; payment_date reset-to-today churn; fs_payments.id re-key orphan) + 3 refuted under verification; the door's record-field bug was the one that escaped review and was caught by the apply itself — empirical-first verification carries the lesson.

---

### DEC-242: The Finance Room — Invoice Substrate (No-TTL Token Link + Edit-Lock-as-Snapshot), the Room over the Books (Glance Derived, Mechanics Excluded), Envelopes v1 as Derived Rules, the Spent-Dedupe Link, and the MWP Step-0 Amendment (2026-06-10)

**Date:** 2026-06-10 (Growth-Queue Session 4)
**Status:** Active — **ON PRODUCTION.** locallane `main` `8e29be6` (merge of feat/finance-room `d2557e4` + hardening `b68ffdd`); migration `20260610120000_finance_room.sql` applied to remote (REHEARSED TWICE first — the whole file + all eleven helper probes executed inside a rolled-back transaction on production via the Management API, the empirical-first law honored BEFORE the apply this time) and probed live. Planted → Living at Doron's walk (his first invoice, start to finish). Paired planner: `MYCELIA-PLAN-FINANCE-ROOM`. Paired builder: `HYPHAE-BUILD-FINANCE-ROOM-COMPLETE`.

**Context:** DEC-241 opened the books — recording every settled payment as double-entry truth — but the books RECORDED without SHOWING, and nothing could ask for money. Session 4 is the ratified first-money flow (Doron is user zero; Bari's estimate-fed flow is the second door): create an invoice → send it (or just record it) → mark it paid → the books post → the Glance updates → the envelopes split.

**Decision:**
- **(a) The invoice substrate** (`fs_invoices`): minimal lifecycle draft → sent → paid; **OVERDUE IS DERIVED** (status=sent AND due_date past), never stored. Two births at the app layer (bill-someone; prefill from a project / accepted-or-signed estimate — the prefill carries the estimate's management-fee/O&P/other adders as their own lines so the bill equals the contract, and PRESERVES line ids for provenance). Numbering INV-YYYY-NNN per business per year (lpad truncation guarded with greatest() — the EST- helper carries the same latent bug, flagged for its own re-issue). The invoice NEVER posts to the ledger — `mark_fs_invoice_paid` mints ONE settled `fs_payments` row (direction=received, type=invoice_payment, party=client, project carried when alive — a soft-deleted project demotes to projectless rather than wedging the money) through `create_fs_payment`, and the DEC-241 bridge posts it in the same transaction. The books schema is untouched; money enters only through its own doors.
- **(b) The no-login link — NO EXPIRY, by deliberate decision.** `fs_invoices.access_token` is the signing-key pattern verbatim: NEVER-class (column-revoked from authenticated; the one member door is `get_invoice_access_token`, admin-gated + audited per access; anon's one door is `get_invoice_portal` — token-scoped, per-field AND per-line-item allowlist, NULL on miss, first-view `viewed_at` stamp as the only anon ink). An invoice link outlives a sign link by nature (clients pay weeks later; a paid invoice keeps rendering as a receipt), and it gates NO state-changing capability — so no TTL. The invalidation levers: the status gate (drafts never render), the **edit lock** (sent/paid invoices are frozen — `update_fs_invoice` raises; THE LOCK IS THE SNAPSHOT, the Assembler's freeze achieved with no copy), and **soft-delete as the kill switch** (`delete_fs_invoice` — draft/sent only, paid refuses as money provenance; the UI's Withdraw). The portal page is `noindex` (a leaked capability URL must not become durably indexable).
- **(c) The Room mounts at the hub** — the ground decided: Finance was already a hub card (sibling to Desk, deliberately absent from DESK_ROOMS), so it mounts at `/businesses/[id]/finance` and the card flips BEING CONNECTED → live (DEC-209). The honest amber pill sits in the header: **records money · doesn't move it yet.** The Glance (in / out / waiting / on hand) is DERIVED from the books, never stored — and **correction mechanics are excluded**: a payment edit's reversal + repost is one real money movement, so the Glance fetches transaction-shaped cash legs and drops reversers, reversed-in-window rows, and superseded payment versions (Doron's walk EDITS a payment; the Glance must not inflate during the proof). The full pen story stays on the books page, where it belongs.
- **(d) Envelopes v1 = STANDING RULES, derived display — deliberately NOT ledger accounts.** The tradeoff, named: real envelope accounts would post book entries for what is v1 a LENS on money-in, force reversal churn on every rule change, and grow the chart with no consumer needing balance-true envelopes; derived rules keep the books pure (only real money posts) and recompute from truth — balances derived, never stored, the books' own discipline. When envelopes MOVE money (a real transfer to tax savings), they graduate to real accounts through `post_ledger_transaction` — the one door. `finance_envelopes` (name + percent + active; active sum ≤ 100 enforced; under-100 remainder honestly unallocated).
- **(e) Glass on the books** — `/finance/books`, member-tier only (RLS gates; the policies NEVER grow crew branches): balances strip + the newest 100 transactions with the pen story explicit — key chips v1 / v1 · reversal / v2, reversed rows struck-through with a "corrected" note, the notch's `unmatched` quietly waiting for the bank's witness.
- **(f) [spent-double-count] RESOLVED — the minimal link.** `fs_payment_covers`: a paid-direction payment declares which material/labor entries it settles (each entry coverable once — partial-unique; replace-set via `set_fs_payment_covers`; clear-only allowed on any direction so a paid→received re-edit dissolves stale links). `computeMoneyTiles` subtracts covered entries' totals — the identity is exact: covers subtract only for SETTLED paid payments, the same set whose dollars ride paidOut. The books never had this problem; the bank notch stays the structural end-state.
- **(g) fs_payments grows up:** `project_id` NULLABLE (the bill-someone payment and the Room-logged expense have no job; the payment is the only path into the books, so it must be mintable without one — per-project Desk surfaces query by project and never see projectless rows; the bridge identity never included it) + `purpose` (the Money-Out tag; free text, outside the reconciler's no-op identity — tagging never churns the books). Both helpers re-issued from their LIVE bodies; payment_date stays PRESERVED BY OMISSION; `update_fs_invoice` generalizes that lesson — EVERY field preserved by omission, NaN refused on money fields, line_items array-validated, cross-tenant provenance pointers refused, FOR UPDATE serializing every lifecycle helper (a send×mark-paid race could otherwise regress a paid invoice and double-mint into the books).
- **(h) UI primitives planted** (`components/ui/` Field / Input / Select / Textarea / Button / Card / FormError — the repo's canonical class strings verbatim): every NEW Room form uses them; sweeping the ~38 existing const-pair copies stays Session 5 (DEC-146).
- **(i) The MWP Step-0 amendment — twice flagged, now law:** a pre-scoped prompt fired by Doron (drafted against a paired MYCELIA-PLAN-* entry, scope + deliverables named) satisfies Step 0's exchange; the reads stay mandatory, the pre-flight summary moves to the debrief, and anything that would CHANGE scope still stops for Doron. Amendment text lives in `MWP-PROTOCOL.md` Step 0 (this DEC is its decision-class pair per spec-amendment-as-commit).

**Rationale:** First money must flow where the truth already lives: the invoice asks, the payment records, the bridge posts, the Room shows — every number derived, nothing stored twice. The no-TTL call follows the threat model (a read-only reference to a living bill, 128-bit capability, kill switch present) rather than cargo-culting expiry; the edit lock replaces a snapshot copy because a frozen original is the cheaper honest snapshot. Envelopes stay rules because the chart should grow only when a consumer needs balance-true accounts — don't over-build.

**Companion to:** DEC-241 (the books this room shows; the bridge mark-paid rides), DEC-240 (the member walls + NEVER-class pattern), DEC-238 (the spine), DEC-237 (the token-portal shape + Resend path), DEC-220 (line attribution stays an estimate-line namespace — invoices don't attribute per-line in v1), DEC-209 (overdue derived; the card flips to a real door), DEC-195 (the adders the prefill must carry), DEC-146 (primitives; one conversion vocabulary), DEC-227 (Hyphae applies; Doron walks), DEC-224/225 (grants discipline incl. the column-enumerated SELECT trap).

**Cross-references:** DEC-139, DEC-146, DEC-167, DEC-195, DEC-203, DEC-209, DEC-218, DEC-220, DEC-224, DEC-225, DEC-227, DEC-228, DEC-232, DEC-237, DEC-238, DEC-240, DEC-241. Record: `context/records/buildings/finance_room.md`. Security: `SECURITY-PRACTICES-AUTH.md` "The Finance Room" entry.

**Evidence:** locallane `d2557e4` (the build: 39 files, +6,627) + `b68ffdd` (review hardening) merged via `8e29be6`, pushed; CI green (branch + main); production healthy. Migration REHEARSED on production twice inside rolled-back transactions (every CREATE + all eleven SD helpers fired to first guard + portal NULL-on-miss) BEFORE the real apply — the DEC-241 door-bug class pre-empted empirically; the apply was uneventful. Verified: tsc exit 0; lint clean (2 pre-existing warnings); build clean; vitest **109/109** (52 new tests: invoices/room/books/financials-dedupe). Live probes: anon 42501 on fs_invoices + finance_envelopes + fs_payment_covers; get_invoice_portal returns NULL on a random token; anon 42501 on get_invoice_access_token + create_fs_invoice; catalog: access_token ABSENT from authenticated's column grants, anon EXECUTE on exactly one function (the portal), all 10 new helpers present, fs_payments.project_id nullable + purpose live. Pre-merge adversarial review (5 lenses, 24 findings) — **the per-finding verification fleet died on a session rate limit; all 24 findings were triaged BY HAND in the main loop** (honest record: nothing was machine-refuted): 17 fixed pre-apply (the send×mark-paid double-mint race; cross-tenant provenance; the missing soft-delete lever; lpad truncation; NaN guards; jsonb-null/non-array line_items; the totals-clobber recurrence; portal line-item over-share; noindex; the settled-covers identity; the Glance mechanics exclusion; prefill adders; revalidation gaps; deleted_at filters; doubled chrome; dead-project wedge; projectless attribution guard), 2 refuted (Button text-white matches the existing destructive copies verbatim — no destructive-foreground token exists; the future-RLS-branch concern matches the Desk gate's accepted posture), 5 accepted-with-note (manual-correction key conventions; cross-month edit window drift — converges at the bank notch; SD-count comment recounted; non-member-on-public-business sees empty chrome like the Desk; re-send re-emails by design).
