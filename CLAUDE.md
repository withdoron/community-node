...
### Base44 `object`-Typed Array Fields — `{items: [...]}` Wrap at Write Boundary (DEC-216, 2026-05-08)

When a Base44 entity field is declared as type `object` and the data shape is logically an array (line items, trade categories, workers, taxonomy presets, phase labels), writes MUST use the `{items: [...]}` wrap pattern. Reads tolerate multiple shapes — bare array, `{items: [...]}` wrap, JSON-string-of-array — via the canonical reader at `src/utils/wrapShape.js` `parseWrappedArray()`. Five-instance evidence: `line_items` (FSEstimate/FSChangeOrder/FSDailyLog), `trade_categories_json` (FieldServiceProfile), `trade_categories_snapshot` (FSEstimate), `workers_json` (FieldServiceProfile), `phase_labels`. Failure mode caught when Phase 2.2 `migrate-trade-categories-snapshot-backfill` initially wrote raw arrays and Base44 rejected validation; commit `49c0bfb` re-wrapped, migration succeeded. See `Spec-Repo/platform/DECISIONS.md` DEC-216 for full rationale and operational rule.

### shadcn `<Select>` Width Discipline (DEC-217, 2026-05-08)

`<SelectTrigger>` defaults to `w-full`. In flex-row layouts where a Select sits alongside form siblings with `flex-1` or `flex-grow` (line item editors, payment forms with description inputs), override to `w-auto min-w-[110px] flex-shrink-0` — otherwise the trigger consumes the row and squeezes siblings to zero width. For standalone form fields (modal pickers, settings panels), `w-full` is correct. Failure mode caught in commit `47e00b8` — line-item description input vanished after Phase 2.2 fix-2 converted kind/trade pickers to shadcn Select with default `w-full`. Five known instances now: `88132a3` standalone correct, `a6f9875` in-row regression, `47e00b8` fix codified, `b282e88` standalone correct (PersonModal), `ff7e741` standalone correct (SubVendorPicker QuickAdd). See `Spec-Repo/platform/DECISIONS.md` DEC-217 for full rationale.
