...
| Desk Home tile drill-throughs | Feature seedling | Surfaced 2026-05-09 | Each numerical tile on the Desk Home (Clients, Active Projects, Estimates, This Month, Received, Team) should open a breakdown popup matching the existing Project Detail Received pattern. Each row in the breakdown clickable to drill into source records. Surfaced when Doron noted `$1,310 Received` tile was opaque (no idea where the number came from). ~2-3 hour focused session when prioritized. Phase 2 follow-up territory. |
| `$1,310 Received` calculation discrepancy audit | Investigation seedling | Surfaced 2026-05-09 | Specific bug in Desk Home Received aggregation. Doron has not recorded $1,310 as received but the tile shows it. Possible causes: aggregation pulling from wrong data source (counting estimated revenue as received), status logic counting pending/scheduled payments, cross-workspace bleed, date-range scoping wrong, or COs/estimate-acceptance double-counting. Investigation becomes feasible once Desk Home tile drill-through ships (the breakdown view will surface what's being counted). ~½-1 hour focused work after drill-through ships. Phase 2 follow-up territory. |
