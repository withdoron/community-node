...
function RulesList({ rules }) {
  return (
    <ul className="space-y-2 pl-1">
      {rules.map((rule, i) => (
        <li key={i} className="flex gap-2 text-sm text-foreground-soft">
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-3 bg-card hover:bg-secondary/80 transition-colors min-h-[44px]"
      >
      ...
      {open && (
        <div className="px-4 py-3 bg-background space-y-4">
    ...
    return (
      <div className="fixed inset-0 z-50 bg-background flex items-center justify-center">
        <div className="text-center">
          <p className="text-muted-foreground">No rules available for this sport/format.</p>
          <button
            type="button"
            onClick={onClose}
    ...
    <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-border px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-foreground font-bold text-lg">NFL FLAG 5v5 Rules</h1>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
        ...
        <div className="pt-4 border-t border-border">
          <h3 className="text-sm font-semibold text-foreground-soft mb-3">Resources</h3>
