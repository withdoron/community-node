    ...
    <Card className="p-5 border-border">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-medium text-foreground">{recommendation.user_name || 'Community Member'}</p>
          <p className="text-xs text-muted-foreground/70">
            {recommendation.created_date && format(new Date(recommendation.created_date), 'MMM d, yyyy')}
          </p>
          {recommendation.service_used && (
            <span className="inline-block mt-1.5 bg-secondary text-muted-foreground text-xs rounded-full px-2 py-0.5">
              {recommendation.service_used}
            </span>
          )}
        </div>
      ...
      <p className="text-sm text-muted-foreground mt-4 leading-relaxed">{recommendation.content}</p>
