...
const TRANSACTION_CONFIG = {
  ...
  reservation: { label: 'Reserved for Event', icon: Minus, color: 'text-primary', bg: 'bg-primary/10' },
  ...
  transfer_out: { label: 'Sent to Member', icon: ArrowRightLeft, color: 'text-primary', bg: 'bg-primary/10' },
  transfer_in: { label: 'Received from Member', icon: ArrowRightLeft, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
  adjustment: { label: 'Adjustment', icon: RefreshCw, color: 'text-muted-foreground', bg: 'bg-muted-foreground/10' },
};
    ...
    <div className="flex items-center justify-between py-3 border-b border-border last:border-0">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground/70">{formattedDate} · {formattedTime}</p>
          {transaction.note && (
      ...
      <div className="text-right">
        <p className={`text-sm font-semibold ${isPositive ? 'text-emerald-500' : 'text-foreground-soft'}`}>
          {isPositive ? '+' : ''}{transaction.amount}
        </p>
        <p className="text-xs text-muted-foreground/70">Balance: {transaction.balance_after}</p>
      </div>
    </div>
  );
}
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between gap-3 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">Joy Coins History</h1>
        ...
        {isLoading || transactionsLoading ? (
          ...
          <Card className="bg-card border-border">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No Joy Coins account yet.</p>
